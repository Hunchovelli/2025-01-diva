...
describe("AaveDIVAWrapper", function () {
  ...
  describe("removeLiquidity", async () => {
    ...
    it("Should remove long token balance amount when using `type(uint256).max` and long < short balance", async () => {
      ...
      await s.aaveDIVAWrapper
        .connect(s.impersonatedSigner)
        .removeLiquidity(r.poolId, ethers.MaxUint256, r.recipient);
    ...
    it("Should revert with `CollateralTokenNotRegistered` error if an invalid poolId is provided", async () => {
      ...
      // ---------
      await expect(
        s.aaveDIVAWrapper.removeLiquidity(poolId, 1, s.owner.address),
      ).to.be.revertedWithCustomError(
        s.aaveDIVAWrapper,
        "CollateralTokenNotRegistered",
      );
    ...
    it("Should revert with `ZeroAddress` error if recipient is the zero address", async () => {
      ...
      await expect(
        s.aaveDIVAWrapper
          .connect(s.impersonatedSigner)
          .removeLiquidity(r.poolId, r.positionTokenAmount, ethers.ZeroAddress),
      ).to.be.revertedWithCustomError(s.aaveDIVAWrapper, "ZeroAddress");
    });
  });
  ...
  describe("redeemWToken", async () => {
    ...
    it("Should emit `WTokenRedeemed` event when redeeming wTokens", async () => {
      // ---------
    ...
    it("Should revert with `ZeroAddress` error if recipient is the zero address", async () => {
      ...
      await expect(
        s.aaveDIVAWrapper
          .connect(s.impersonatedSigner)
          .redeemWToken(wTokenAddress, wTokenBalance, ethers.ZeroAddress),
      ).to.be.revertedWithCustomError(s.aaveDIVAWrapper, "ZeroAddress");
    });
  });
  ...
  describe("batchRedeemPositionToken", () => {
